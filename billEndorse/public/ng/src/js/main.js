  ...
  .controller('AppCtrl', ['$scope', '$translate', '$localStorage', '$window','$state', 
    function(              $scope,   $translate,   $localStorage,   $window ,$state) {
      ...
      $scope.$on('signinToAppCtrlForRole', function(event,role) {
    	    console.log('get login role as ', role);	   
    	    $scope.app.role = role;
    	  });
      
      $scope.$on('signinToAppCtrlForLoginName', function(event,loginname) {
  	    console.log('get loginName  as ', loginname);	   
  	    $scope.app.loginname = loginname;
  	  });
